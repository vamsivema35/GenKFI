...
{
    public partial class unassignedBatches : System.Web.UI.Page
	{
		//string strProgCode;
		//string strProgLang;
		string strCurrPage;
		...
		protected void Page_Load( object sender, EventArgs e )
		{
			...
			strCurrPage = "Unassigned Batches";
			//@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#
			...
			if( !IsPostBack )
			{
				...
				usersDDL.Items.Add( new ListItem( "Select a User", "-1" ) );
		...
		private void LoadBatches()
		{
			...
			try
			{
				...
				strSQL += " AND a.fstrBatchUser LIKE 'BATCH%'";
                ...
                strSQL += ", 'Not Assigned' as BatchStatus";
				...
				strSQL += " AND a.fstrTaxProgram IN ( '" + Session[ "tpSelHID" ].ToString().Replace( "ALL;", "" ).Replace( ";", "', '" ) + "' )";
				strSQL += " AND a.fstrBatchUser LIKE 'BATCH%'";
				strSQL += " ORDER BY a.fdtmBatchDate, a.fstrBatchID";
				gc.DoSelect( strSQL );
				dt = new DataTable();
				numRows = gc.ReadSelect( out dt );
				...
				if( numRows > 0 )
				{
					...
					strSQL += " WHERE fstrBatchStatus = 'O'";
					strSQL += " AND fstrTaxProgram IN ( '" + Session[ "tpSelHID" ].ToString().Replace( "ALL;", "" ).Replace( ";", "', '" ) + "' )";
					strSQL += " AND fstrBatchUser = 'BATCH'";
					strSQL += " GROUP BY fstrForm, fstrBatchType, fstrImgSrc";
					strSQL += " UNION ";
					strSQL += "SELECT fstrForm as TaxYear";
					...
					strSQL += " WHERE fstrBatchStatus = 'O'";
					strSQL += " AND fstrTaxProgram IN ( '" + Session[ "tpSelHID" ].ToString().Replace( "ALL;", "" ).Replace( ";", "', '" ) + "' )";
					strSQL += " AND fstrBatchUser = 'BATCH2D'";
					strSQL += " GROUP BY fstrForm, fstrBatchType, fstrImgSrc";
					...
					detailRPT.DataSource = null;
					detailRPT.DataBind();
					if( numRows > 0 )
					{
						detailRPT.DataSource = dt;
						detailRPT.DataBind();
		...
		protected void OnClick_assignBTN( object sender, EventArgs e )
		{
			string strChecked = Request.Form[ "assignMe" ];
			// nothing checked, no user selected, leave
			if( strChecked == null || usersDDL.SelectedValue == "-1" )
				return;
			...
			if( strChecked != "" )
			{
				...
				try
				{
					...
					foreach( string s in strCheckedList )
					{
						...
						string[] info;
						info = s.Split( ';' );
						statement = "UPDATE tblNM_BatchInfo SET fstrBatchUser = '" + usersDDL.SelectedItem.Text + "', ";
						...
						statement += "CONVERT( VARCHAR(10), fdtmBatchDate, 101 ) = '" + info[ 1 ] + "' AND ";
						statement += "fstrImgSrc = '" + info[ 2 ] + "';";
