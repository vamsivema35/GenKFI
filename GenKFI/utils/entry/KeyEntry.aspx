...
<asp:Content ID="headerContent" ContentPlaceHolderID="headerHolder" runat="server">
		...
		<div id="divHeader" style="position:absolute;top:0px;left:0px;height:72px;width:100%;" runat="server">
			...
			<table style="left: 325px;position: relative;">
				<tr>
					<td><asp:Label runat="server" ID="batchIDLBL" CssClass="headerInput" Width="75" Text="Batch ID:" ></asp:Label></td>
					<td><asp:Label runat="server" ID="batchIDTXT" CssClass="headerInput" Width="75" ></asp:Label></td>
				</tr>
				<tr>
					<td><asp:Label runat="server" ID="batchDateLBL" CssClass="headerInput" Width="75" Text="Batch Date:" ></asp:Label></td>
					<td><asp:Label runat="server" ID="batchDateTXT" CssClass="headerInput" Width="75" ></asp:Label></td>
				</tr>
				<tr>
					<td><asp:Label runat="server" ID="itemNoLBL" CssClass="headerInput" Width="75" Text="Item No:" ></asp:Label></td>
					<td><asp:Label runat="server" ID="itemNoTXT" CssClass="headerInput" Width="75" ></asp:Label></td>
				</tr>
			</table>
	...
	<asp:Content runat="server" ID="scriptingContent" ContentPlaceHolderID="scriptingHolder">
		...
		<script type="text/javascript">
		    ...
		    function rtnMavCap(MavKey) {
		        // Check F12 Key Press  - Change to Select / Focus
		      //  if (MavKey == "F12") {
		      //      document.getElementById("<-%=btnSub.ClientID%>").click();
		            //    $('#<-%=btnSub.ClientID%>').click();
		      //  }
                // Check for PgUp, PgDn, Home, End keys if Mavro
		        if ($("#<%= ( (HiddenField)Master.FindControl("hidmvImgSrc")).ClientID%>").val() == "MAVRO")
		        {
		            $("#<%= ( (HiddenField)Master.FindControl("hidMavKey")).ClientID%>").val(MavKey);
...
//                                $('#txtTpName' + repeaterIndex).val(pData['fstrListFormatName']);
							...
							*/
							...
							/*
							if( $('#txtStreet') != null )
								$('#txtStreet').val(data[ 'secondary' ]['fstrStreet']);
							if( $('#txtCity') != null )
								$('#txtCity').val(data[ 'secondary' ]['fstrCity']);
							if( $('#txtState') != null )
								$('#txtState').val(data[ 'secondary' ]['fstrState']);
							if( $('#txtZipCode') != null )
								$('#txtZipCode').val(data[ 'secondary' ]['fstrZip']);
							if( $('#txtCountry') != null )
								$('#txtCountry').val(data[ 'secondary' ]['fstrCountry']);
							*/
						}
					});
			}
			...
			$(document).ready(function(){
				...
				$('#divImageDrag').on("drag", function(event, ui){
					...
					$("#dragToffsetHID").val($("#divImageDrag").css("top").replace("px", ""));
					$("#dragLoffsetHID").val($("#divImageDrag").css("left").replace("px", ""));
				});
				$("#divImageDrag").mouseover(function() { $("#divImageDrag").css("cursor", "move"); });
				$('#<%=btnSub.ClientID %>').focusin(function(){
					//$('#divMarker').stop().css("background-color", "#00FF00").animate({ backgroundColor: "#FF4500"}, 800).delay(100).stop().css("background-color", "#00FF00").animate({ backgroundColor: "#FF4500"}, 800);
					//$('#divMarker').fadeOut(500).fadeIn(500);
					$('#divMarker').effect("pulsate", { times:3 }, 500, function(){
						$('#divMarker').css('opacity','0.35');});
					$('#<%=btnSub.ClientID %>').css('background-color','#00AAFF');
				});
